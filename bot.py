#!/usr/bin/env python3
"""
Setka Cup Betting Bot - Version Corrigée
"""
...
# Noms EXACTS des sections tels qu'ils apparaissent sur la page
COMP_SECTION_NAMES = {
...
def fetch_matches_today(competition_key: str) -> list:
    """
    Structure réelle de la page (confirmée par debug) :
      'DD.MM HH:MM'                  <- timestamp = séparateur de bloc
      'Setka Cup. Czech Republic'    <- 1ère ligne du bloc = nom exact compétition
      'W1: ...'  'W2: ...' ...       <- données du match

    On split le HTML sur les timestamps, puis on vérifie que
    la 1ère ligne de texte du bloc est EXACTEMENT le nom voulu.
    Résultat : zéro contamination entre compétitions.
    """
    ...
    parts = re.split(r'(\d{2}\.\d{2}\s+\d{2}:\d{2})', full_html)

    i = 1
    while i < len(parts) - 1:
        header     = parts[i]
        block_html = parts[i + 1]
        i += 2
        ...
        # Vérification EXACTE : 1ère ligne = nom de la compétition
        if block_lines[0] != section_name:
            continue

        match = parse_match_block(block_soup, block_text, date_part, time_part, competition_key)
        if match:
            matches.append(match)
            log.info(f"[{COMP_LABELS.get(competition_key)}] Match : {match['player1']} vs {match['player2']} à {time_part}")

    if not matches:
        log.info(f"[{COMP_LABELS.get(competition_key)}] Aucun match '{section_name}' aujourd'hui ({today_str}).")
...
def format_alert(a: dict) -> str:
    ...
    p1_odds = str(a["fav_odds"]) if a["favorite"] == a["player1"] else str(a["und_odds"])
    p2_odds = str(a["fav_odds"]) if a["favorite"] == a["player2"] else str(a["und_odds"])
...
def process_alert(match: dict):
    ...
    if not IGNORE_ODDS_FILTER and not (MIN_FAVORITE_ODDS <= fav_odds <= MAX_FAVORITE_ODDS):
        log.info(f"Cote hors filtre ({fav_odds}) — ignoré : {match['player1']} vs {match['player2']}")
        return
    ...
    while True:
        try:
            for comp in COMPETITIONS:
                ...
                log.info(f"[{COMP_LABELS.get(comp, comp)}] {len(matches)} match(s) trouvé(s)")
